...
static __device__ float computeMitchellFilterSampleContribution(float2 sample, uint2 current_launch_index, float2 scaled_mitchell_filter_radius)
{
	...
	//int filter_table_width_offset_x = min(static_cast<int>(normalized_dist.x * mitchell_filter_half_table_width), (mitchell_filter_half_table_width - 1));
	//int filter_table_width_offset_y = min(static_cast<int>(normalized_dist.y * mitchell_filter_half_table_width), (mitchell_filter_half_table_width - 1));
	...
	//{
	//	rtPrintf("Normalized distance: [ %f , %f ]\n", normalized_dist.x, normalized_dist.y);
	//	rtPrintf("Filter table width: %d\n", mitchell_filter_table_width);
	//	rtPrintf("Current filter table width offset: [ %d , %d ]\n", filter_table_width_offset.x, filter_table_width_offset.y);
	//}
	...
	//if (current_launch_index.x == 256 && current_launch_index.y == 256)// || normalized_dist_length > 0.5f)
	//{
	//	rtPrintf("\nNormalized distance: [ %f , %f ] and its length: %f\n", normalized_dist.x, normalized_dist.y, normalized_dist_length);
	//	rtPrintf("Filter table width: %d\n", mitchell_filter_table_width);
	//	rtPrintf("Current filter table width offset: [ %d , %d ] and filter weight: %.3f\n\n", filter_table_width_offset.x, filter_table_width_offset.y, mitchell_filter_weight);
	//}

	//if (current_launch_index.x == 256 && current_launch_index.y == 256)// || normalized_dist_length > 0.5f)
	//{
	//	rtPrintf("Filter table weight: %f\n", mitchell_filter_weight);
	//}
																				...
																				buffer<float4, 2>* filter_x_sample_sum_buffer)
{
	...
	for (int x = 0; x < scaled_radius.x * 2 + 1; x++)
	{
		for (int y = 0; y < scaled_radius.y * 2 + 1; y++)
		{
			...
			//if (current_launch_index.x == 256 && current_launch_index.y == 256)// || normalized_dist_length > 0.5f)
			//{
			//	//rtPrintf("\nCurrent lauch index top left: [ %d , %d ]\n", current_launch_index_top_left.x, current_launch_index_top_left.y);
			//	//rtPrintf("Scaled mitchel filter radius: [ %f , %f ]\n\n", scaled_radius.x, scaled_radius.y);

			//	atomicExch(&(*filter_x_sample_sum_buffer)[neighborhood_idx].x, 1000.0f);
			//	if (neighborhood_idx.x == current_launch_index.x && neighborhood_idx.y == current_launch_index.y)
			//	{
			//		//rtPrintf("neighborhood_idx [ x , y ]: [ %d , %d ]\n", x, y);
			//		atomicExch(&(*filter_x_sample_sum_buffer)[neighborhood_idx].y, 10000.0f);
			//	}
			//}
		}
	}
